...
class TrackingFragment: Fragment(R.layout.fragment_tracking) {
    ...
    private fun showCancelTrackingDialog() {
        val dialog = MaterialAlertDialogBuilder(requireContext(), R.style.AlertDialogTheme)
            ...
            .setMessage("Are you sure you want to cancel current run and delete all its data?")
    ...
    private fun addLatestPolyLine() {
        if(pathPoints.isNotEmpty() && pathPoints.last().size > 1) {
            val preLastLatLng = pathPoints.last()[pathPoints.last().size - 2] //secondlast element of last list
            val lastLatLng = pathPoints.last().last() //last element of last list
            val polylineOptions = PolylineOptions()
                .color(POLYLINE_COLOUR)
                .width(POLYLINE_WIDTH)
                .add(preLastLatLng)
